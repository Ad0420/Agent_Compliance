"""Unit tests for ``vera.verify.proof_payload`` (Wave 3C.2).
...
Module renamed from ``test_verify_proof.py`` during the Wave 3C.1 rebase
to coexist with 3C.1's ``test_verify_proof.py`` (which tests the
split-layout ``vera.verify.proof`` from the subpackage refactor). The
two test suites cover the same Merkle-fold math through different
public APIs; both ride together.
"""
...
from vera.verify.proof_payload import (
    ALGO_HMAC_SHA256,
    ALGO_RSA_PSS_SHA256,
