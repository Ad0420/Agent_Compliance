...
| **`vera verify --merkle-proof` — tampered leaf** | unit: flip a bit in `leaf_hash` → verification fails with `merkle_proof_invalid`; exit 1. | server-side covered (Wave 3B.2 — `verify_proof_payload` rejects in `tests/test_merkle_proof_endpoint.py::test_tampered_leaf_hash_fails_verification`); CLI surface defers to Wave 3C |
| **`vera verify --merkle-proof` — tampered sibling** | unit: flip a bit in `proof_hashes[i]` → verification fails. | server-side covered (Wave 3B.2 — `tests/test_merkle_proof_endpoint.py::test_tampered_sibling_hash_fails_verification`); CLI surface defers to Wave 3C |
| **`vera verify --merkle-proof` — wrong root** | unit: replace `root` with a different valid checkpoint root → verification fails (root mismatch). | server-side covered (Wave 3B.2 — `tests/test_merkle_proof_endpoint.py::test_wrong_root_fails_verification`); CLI surface defers to Wave 3C |
