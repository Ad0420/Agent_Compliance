...
| **`vera verify --merkle-proof` — happy path** | integration: proof file downloaded from endpoint → CLI verifies offline against customer S3 mirror's KMS public-key history; exit 0. | covered (Wave 3C.2 — `sdk/vera/_cli_verify.py::verify_merkle_proof_for_record` + `sdk/vera/verify/proof.py::verify_proof_payload`; `sdk/tests/test_cli_verify_and_export.py::test_verify_merkle_proof_happy_path` + `test_cli_verify_calls_endpoint_and_exits_zero`) |
| **`vera verify --merkle-proof` — tampered leaf** | unit: flip a bit in `leaf_hash` → verification fails with `merkle_proof_invalid`; exit 1. | covered (Wave 3C.2 — `sdk/tests/test_verify_proof.py::test_tampered_leaf_fails` + `test_full_verify_tampered_leaf_fails_with_path_mismatch_reason` + CLI exit-code path `test_verify_merkle_proof_tampered_leaf_returns_exit_1`) |
| **`vera verify --merkle-proof` — tampered sibling** | unit: flip a bit in `proof_hashes[i]` → verification fails. | covered (Wave 3C.2 — `sdk/tests/test_verify_proof.py::test_tampered_sibling_fails` + `test_full_verify_tampered_sibling_fails` + CLI `test_verify_merkle_proof_tampered_sibling_returns_exit_1`) |
| **`vera verify --merkle-proof` — wrong root** | unit: replace `root` with a different valid checkpoint root → verification fails (root mismatch). | covered (Wave 3C.2 — `sdk/tests/test_verify_proof.py::test_wrong_root_fails` + `test_full_verify_wrong_root_fails` + CLI `test_verify_merkle_proof_wrong_root_returns_exit_1`) |
| **`vera verify --merkle-proof` — bad KMS signature** | unit: keep proof valid but tamper KMS signature → verification fails with `signature_invalid`. | covered (Wave 3C.2 — `sdk/tests/test_verify_proof.py::test_full_verify_bad_signature_fails` + `test_rsa_pss_happy_and_tampered` + CLI `test_verify_merkle_proof_bad_signature_returns_exit_1`) |
...
| **KMS rotation between checkpoints — proof still verifies** | integration: rotate KMS key between checkpoint N and N+1; proof for a record in N verifies via `vera verify --merkle-proof` using the key history table (per Eng F10). | covered (Wave 3C.2 — backend-side history join lands the historical algorithm + public_key_pem in the proof payload via `services/merkle_proof.build_proof` reading `kms_keys` via `get_key_by_id`; SDK `verify_proof_payload` consumes those fields without ever re-querying Vera, so a rotated key's old proofs still verify offline. RSA-PSS path exercised by `sdk/tests/test_verify_proof.py::test_rsa_pss_happy_and_tampered`) |
