"""Settings for the TriageGuard backend.
...
class Settings:
    ...
    signed_in_as: str = field(
        default_factory=lambda: os.environ.get(
            "TRIAGEGUARD_USER_LABEL", "Nurse Rivera, RN"
        )
    )
    ...
    db_url: str = field(
        default_factory=lambda: os.environ.get(
            "TRIAGEGUARD_DB_URL",
            "sqlite+aiosqlite:///./triageguard_backend.db",
        )
    )
    ...
    vera_customer_slug: str = "triageguard"

    # CORS — frontend dev server lives at 3002 to avoid clashing with
    # ScribeMD (3001) and Vera (3000).
    ...
    cookie_name: str = "triageguard_session"
    cookie_max_age_seconds: int = 60 * 60 * 8  # 8h working day

    # Nurse review wait — soft cap on how long a workflow blocks for a
    # decision. Mirrors ScribeMD's `approval_timeout_seconds` knob; tests
    # set it to 5 to prevent teardown hangs.
    review_timeout_seconds: int = int(
        os.environ.get("TRIAGEGUARD_REVIEW_TIMEOUT_SECONDS", "300")
    )
