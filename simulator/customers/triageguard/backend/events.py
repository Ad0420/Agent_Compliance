...
Each session has its own queue per subscriber. The workflow runner
publishes; the SSE route subscribes and forwards. Events are also
persisted to the session row so reconnecting clients can replay
history before the live tail.
...
class EventBus:
    """One bus per process; one set of subscriber queues per session."""

    def __init__(self) -> None:
        # session_id -> list of asyncio.Queue, one per live subscriber
        self._subscribers: dict[str, list[asyncio.Queue[Event | None]]] = defaultdict(list)
        # Sessions that have closed their stream (terminal event published).
        self._closed: set[str] = set()

    def publish(self, session_id: str, event_type: str, payload: dict) -> None:
        """Fan an event out to every live subscriber. Safe to call from
        the running event loop (route handlers, lifespan tasks).

        Note: this is the *event-loop-side* publish. Workers running in
        a thread must hop back via `loop.call_soon_threadsafe(bus.publish, ...)`.
        """
    ...
    async def subscribe(self, session_id: str) -> AsyncIterator[Event]:
        """Yield events for `session_id` until the stream is closed."""
...
def reset_bus_for_tests() -> None:
    """Replace the module-level bus with a fresh one. Tests call this
    between cases so subscribers from one test don't leak into another."""
    global _BUS
    _BUS = EventBus()
