...
/**
 * `useSession(id)` — one-shot fetch that auto-polls every 1s while the
 * session is non-terminal, then stops.
 *
 * `useStartSession()` — exposes a `start(input)` function for the view
 * layer's "start triage session" affordance.
 *
...
export function useSession(id: string | null): UseSessionResult {
  ...
  React.useEffect(() => {
    ...
    const fetchOnce = async (): Promise<SessionSnapshot | null> => {
      try {
...
export function useStartSession(): UseStartSessionResult {
  ...
  const start = React.useCallback(
    async (input: StartSessionInput): Promise<{ id: string }> => {
      setIsStarting(true);
      setError(null);
      try {
