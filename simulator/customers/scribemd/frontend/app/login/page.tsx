 ...
 *
 * Single centered card. The clinician types a passkey, hits "Sign in",
 * and on success lands on `/encounter`. No "forgot passkey" link, no
 * SSO buttons — the demo bench keeps the surface minimal so the next
 * screen (the live encounter) is the focal point.
 */
...
export default function LoginPage() {
  ...
  // to the workspace — no point asking the clinician to re-authenticate.
  React.useEffect(() => {
      ...
      <Card elevation="lg" className="w-full max-w-md">
        <CardContent className="pt-8 pb-7 px-7">
          ...
          <p className="mt-1.5 font-sans text-sm text-[var(--ink-3)]">
            Enter your passkey to open the workspace.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
            ...
            {error && (
              <div
                id="login-error"
                role="alert"
                className="rounded-xl bg-[var(--coral-soft)] px-3.5 py-2.5 font-sans text-sm text-[var(--coral)] ring-1 ring-inset ring-[rgba(180,35,24,0.22)]"
              >
                {error}
              </div>
            )}
