 ...
 * "is the clinician signed in?". We assert the /me probe lifecycle and the
...
describe("useAuth — /me probe on mount", () => {
  it("exposes signedInAs once /api/auth/me resolves", async () => {
    mockFetchSequence([
      () => jsonResponse({ signed_in_as: "Dr. Adams" }),
    ]);
    ...
    expect(result.current.signedInAs).toBe("Dr. Adams");
  });
...
describe("useAuth — login / logout", () => {
  ...
  it("logout() clears signedInAs", async () => {
    ...
    const { result } = renderHookWithAuth(() => useAuth());
    await waitFor(() => expect(result.current.signedInAs).toBe("Dr. Adams"));
